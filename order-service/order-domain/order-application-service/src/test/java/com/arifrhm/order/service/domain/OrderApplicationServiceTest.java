...
import com.arifrhm.order.service.domain.dto.create.CreateOrderResponse;
import com.arifrhm.order.service.domain.dto.create.OrderAddress;
import com.arifrhm.order.service.domain.dto.create.OrderItem;
...
public class OrderApplicationServiceTest {
    ...
    public void init() {
        ...
        User user = User.builder()
                .userId(new com.arifrhm.domain.valueobject.UserId(USER_ID))
                .name(USER_NAME)
                .email(USER_EMAIL)
                .role(com.arifrhm.domain.valueobject.UserRole.CUSTOMER)
                .isActive(true)
                .token("jwtToken")
                .build();
        user.initializeUser();
        when(userRepository.findById(new com.arifrhm.domain.valueobject.UserId(USER_ID)))
        ...
        Warehouse warehouse = Warehouse.builder()
                .warehouseId(new com.arifrhm.domain.valueobject.WarehouseId(UUID.fromString(WAREHOUSE_ID)))
        ...
        Stock stock = new Stock(
                ...
                new com.arifrhm.domain.valueobject.WarehouseId(UUID.fromString(WAREHOUSE_ID)),
                new com.arifrhm.domain.valueobject.ProductId(UUID.fromString(PRODUCT_ID)),
                10 // Available stock
        );
        ...
        Order order = Order.builder()
                ...
                .price(new com.arifrhm.domain.valueobject.Money(createOrderCommand.getPrice()))
...
//        when(request.getAttribute("userId")).thenReturn(USER_ID.toString());
    ...
    public void testCreateOrder_Success() {
        ...
        assertEquals(com.arifrhm.domain.valueobject.OrderStatus.PENDING, response.getOrderStatus(), "Order status should be PENDING");
        ...
        assertEquals(new com.arifrhm.domain.valueobject.UserId(USER_ID), capturedOrder.getUserId(), "User ID should match");
        assertEquals(new com.arifrhm.domain.valueobject.WarehouseId(UUID.fromString(WAREHOUSE_ID)), capturedOrder.getWarehouse().getId(), "Warehouse ID should match");
        assertEquals(new com.arifrhm.domain.valueobject.Money(PRODUCT_PRICE.multiply(new BigDecimal(PRODUCT_QUANTITY))), capturedOrder.getPrice(), "Total price should match");
        assertEquals(1, capturedOrder.getItems().size(), "There should be one order item");
        assertEquals(new com.arifrhm.domain.valueobject.ProductId(UUID.fromString(PRODUCT_ID)), capturedOrder.getItems().get(0).getProduct().getId(), "Product ID should match");
    ...
    public void testCreateOrder_UserNotFound() {
        ...
        // Assertions
        assertEquals("Could not find user with user id: " + new com.arifrhm.domain.valueobject.UserId(USER_ID).getValue(), exception.getMessage(), "Exception message should match");
    ...
    public void testCreateOrder_InsufficientStock() {
        ...
        // Assertions
        assertEquals("Order item quantity: 2 is greater than total stock: 1 for product " + new com.arifrhm.domain.valueobject.ProductId(UUID.fromString(PRODUCT_ID)).getValue(), exception.getMessage(), "Exception message should match");
